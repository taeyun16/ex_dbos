defmodule ExDbos.Idempotency do
  ...
  defp mark_succeeded(client, request_key, payload) do
    ...
    case SQL.query(client.repo, sql, [encoded_payload, request_key]) do
      ...
      {:error, reason} ->
        raise RuntimeError, message: "failed to mark idempotency success: #{inspect(reason)}"
    end
  end
  ...
  defp mark_failed(client, request_key, error_message) do
    ...
    case SQL.query(client.repo, sql, [message, request_key]) do
      ...
      {:error, reason} ->
        raise RuntimeError, message: "failed to mark idempotency failure: #{inspect(reason)}"
    end
  end

  defp maybe_cleanup(client, opts) do
    cleanup_interval =
      Keyword.get(opts, :cleanup_interval_seconds, @default_cleanup_interval_seconds)
  ...
  defp ensure_cleanup_table do
    case :ets.whereis(@cleanup_table) do
      :undefined ->
        :ets.new(@cleanup_table, [:named_table, :public, :set, read_concurrency: true])
